...
class Strings {
  ...
  String _localizedValue(String key) {
    Map<String, String> localizedStrings = {
      ...
      'login_welcome': isKr() ? '환영합니다! 세부 사항을 입력해 주세요' : 'Welcome! Please enter your details',
      'login_no_account': isKr() ? '계정이 없으신가요?' : 'No account?',
      'login_email_correct': isKr() ? '이것은 유효한 이메일 주소입니다' : 'This is a valid email address',
      'login_email_invalid': isKr() ? '이메일 주소가 올바르지 않습니다' : 'Email address entered incorrectly',
      'login_pw_invalid': isKr() ? 'a-z, A-Z, 0-9, 4에서 20자' : 'a-z, A-Z, 0-9, 4 to 20 characters',

      'signup_full_name_hint': isKr() ? '이름을 입력해 주세요' : 'Please enter your name',
      'signup_business_name': isKr() ? '사업체 이름' : 'Business name',
      'signup_business_name_hint': isKr() ? '사업체 이름을 입력해 주세요' : 'Please enter a business name',

      'main_navigation_menu_schedules': isKr() ? '스케줄' : 'Schedules',
      'main_navigation_menu_playlists': isKr() ? '플레이리스트' : 'Playlists',
      'main_navigation_menu_screens': isKr() ? '스크린' : 'Screens',
      'main_navigation_menu_media': isKr() ? '미디어' : 'Media',
      'main_navigation_menu_my': isKr() ? '나의' : 'My',

      'scan_qr_title': isKr() ? 'QR 코드 스캔' : 'Scan QR code',
      'scan_qr_enter_pin_code': isKr() ? 'PIN 코드 입력' : 'Enter PIN code',
      'scan_qr_description': isKr() ? 'TV 화면에 보이는 QR 코드를 스캔하세요' : 'Scan the QR code you see\non the TV screen',

      'apply_screen_title': isKr() ? '스크린에 적용' : 'Apply to Screens',

      'create_playlist_title': isKr() ? '플레이리스트 생성' : 'Create playlist',
      'create_playlist_title_input': isKr() ? '제목을 입력해 주세요' : 'Please enter a title',

      'select_playlist_title': isKr() ? '플레이리스트 선택' : 'Playlist',

      'destination_folder_title': isKr() ? '목적지 폴더 선택' : 'Select destination folder',
      'destination_folder_root': isKr() ? '미디어' : 'Media',

      'create_schedule_title': isKr() ? '스케줄 생성' : 'Create schedule',
      'create_schedule_title_input': isKr() ? '제목을 입력해 주세요' : 'Please enter a title',
      ...
      'create_schedule_add_playlist': isKr() ? '플레이리스트 추가' : 'Add Playlist',
      'create_schedule_change_playlist': isKr() ? '플레이리스트 변경' : 'Change Playlist',
      'create_schedule_new_playlist': isKr() ? '새 플레이리스트' : 'New playlist',

      'edit_playlist_title': isKr() ? '플레이리스트 편집' : 'Edit playlist',
      'edit_schedule_title': isKr() ? '스케줄 편집' : 'Edit schedule',
      ...
      'my_page_profile_business_name': isKr() ? '사업체 이름' : 'Business name',
      'my_page_profile_delete_account': isKr() ? '계정 삭제' : 'Delete account',
      'delete_account_title': isKr() ? '계정 삭제' : 'Delete account',
      'delete_account_description': isKr() ? '계정을 삭제하시겠습니까?' : 'Are you sure you want to delete your account?',
      'delete_account_description1': isKr()
          ? '계정을 삭제하면 모든 정보와 데이터가 삭제됩니다.\n삭제하시려면 주의 사항을 확인하고 진행해 주세요'
      ...
      'delete_account_reason_title': isKr() ? '계정 삭제 이유' : 'Reason for deleting your account',
      ...
      'blank_message_description_add_screen': isKr()
          ? '현재 저장된 화면 목록이 없습니다\nQR 코드를 통해 화면을 등록해 주세요'
          : 'There is currently no saved screen list\nPlease register the screen through QR code',
      'blank_message_description_add_canvas':
          isKr() ? '현재 저장된 캔버스가 없습니다\n새로운 캔버스를 만들어 주세요' : 'No canvas currently saved\nPlease make a new canvas',
      'blank_message_description_new_schedule': isKr()
          ? '현재 저장된 스케줄이 없습니다\n스케줄을 등록하거나 생성해 주세요'
          : 'There are currently no saved schedules\nPlease register or create a schedule',
      'blank_message_description_new_playlist': isKr()
          ? '현재 저장된 플레이리스트가 없습니다\n플레이리스트를 등록하거나 생성해 주세요'
          : 'There are currently no saved playlists\nPlease register or create a playlist',
      ...
      'popup_rename_screen_hint': isKr() ? '화면 이름 입력' : 'Enter Screen name',
      ...
      'common_full_name': isKr() ? '전체 이름' : 'Full name',
      'common_updated': isKr() ? '업데이트됨' : 'Updated',
      'common_back': isKr() ? '뒤로' : 'Back',
      ...
      'message_operation_timeout': isKr() ? '작업이 너무 지연되고 있습니다.\n몇 분 후에 다시 시도해주세요' : 'Operation is being delayed too much.\nPlease try again in a few minutes',
      'message_not_found_user': isKr() ? '사용자 정보를 찾을 수 없습니다' : 'User information not found',
      'message_add_media_content': isKr() ? '미디어 콘텐츠를 추가해주세요' : 'Please add media content',
      'message_api_success': '',
      'message_temp_login_fail': isKr() ? '일시적인 오류가 발생했습니다\n다른 로그인 방법을 사용해주세요' : 'A temporary failure has occurred.\nPlease use a different login method',
      'message_time_setting_precede': isKr() ? '종료 시간은 시작 시간을 앞서갈 수 없습니다' : 'End time cannot precede start time',
      'message_time_setting_duplicated': isKr() ? '중복된 시간을 수정해주세요' : 'Please correct the duplicate time',
      'message_apply_screen_success': isKr() ? '스크린에 성공적으로 적용되었습니다' : 'Successfully applied to screens',
      'message_register_screen_success': isKr() ? '스크린에 성공적으로 추가되었습니다' : 'Successfully added to the screens',
      'message_remove_playlist_success': isKr() ? '플레이리스트가 성공적으로 삭제되었습니다' : 'Playlist deleted successfully',
      'message_register_playlist_success': isKr() ? '플레이리스트가 성공적으로 등록되었습니다' : 'Playlist registered successfully',
      'message_update_playlist_success': isKr() ? '플레이리스트 정보가 변경되었습니다' : 'Playlist information has been changed',
      'message_add_media_in_playlist_success': isKr() ? '플레이리스트에 미디어를 성공적으로 추가했습니다' : 'Successfully Add Media to Playlist',
      'message_remove_schedule_success': isKr() ? '스케줄이 성공적으로 삭제되었습니다' : 'Schedule deleted successfully',
      'message_register_schedule_success': isKr() ? '스케줄이 성공적으로 등록되었습니다' : 'Schedule registered successfully',
      'message_update_schedule_success': isKr() ? '스케줄 정보가 변경되었습니다' : 'Schedule information has been changed',
      'message_remove_media_success': isKr() ? '미디어 파일이 성공적으로 삭제되었습니다' : 'Media file deleted successfully',
  ...
  String get createScheduleDefaultPlaylistTitleMorning => _localizedValue('create_schedule_default_playlist_title_morning');
  String get createScheduleDefaultPlaylistTitleLunch => _localizedValue('create_schedule_default_playlist_title_lunch');
  String get createScheduleDefaultPlaylistTitleDinner => _localizedValue('create_schedule_default_playlist_title_dinner');
  ...
  String select_media_file_title(int count) => isKr() ? "$count페이지" : "$count pages";

  // Count Strings with Placeholders
  String count_playlist(int count) => "$count playlists";

  String count_pages(int count) => "$count pages";

  String count_folder(int count, String size) => "$count Files$size";

  String type_media_size(String type, String size) => "$type - $size";

  String message_move_media_success(String folder) => "File moved to $folder";
}
